...
const GloVendorAPI = {
  ...
  getCurrentUser: async () => {
    ...
    switch (role) {
      ...
      case "AGGREGATOR":
        endpoint = "/api/aggregators/me";
        break;

      case "SUBVENDOR":
        endpoint = "/api/subvendors/me";
        break;

      case "RETAILER":
        endpoint = "/api/retailers/me";
        break;
  ...
  getSubvendorById: async (id) => {
    // 1. Sanitize the ID: Remove accidental quotes or whitespace
    // e.g., changes '"12345"' to '12345'
    const cleanId = String(id).replace(/['"]+/g, "").trim();

    // 2. Debug: Log the exact path being requested
    console.log(`[API Request] GET /api/subvendors/${cleanId}`);

    try {
      const res = await axiosInstance.get(`/api/subvendors/${cleanId}`);
      return res.data;
    } catch (error) {
      // If it is a 404, it means this specific ID does not exist in the DB
      if (error.response && error.response.status === 404) {
        console.error(`Subvendor with ID ${cleanId} not found.`);
      }
      throw error;
    }
  },
    ...
    try {
      ...
      return Array.isArray(plans)
        ? plans.map((p) => ({
            ...
            // ✅ Updated: Backend now sends 'planName'
            name: p.planName || p.name || "Unknown Plan",

            // ✅ Updated: Backend sends 'priceNaira'
            price: p.priceNaira || 0,

            // ✅ Updated: Validity is now a String (e.g., "30", "1 Hour", "Sat & Sun")
            // We map it to 'validity' instead of 'validityDays' to avoid type confusion
            validity: p.validity || "N/A",

            // ✅ New Fields from CSV
            dataVolume: p.totalDataVolume || "",
            ...
            // Legacy field (kept as you re-added it to the entity)
            dataServices: p.dataServices || "Standard",
