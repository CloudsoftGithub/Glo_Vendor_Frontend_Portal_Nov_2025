...
export default function SubvendorDashboard() {
  ...
  const handleBuyPlan = async (plan) => {
    ...
    try {
      ...
      mutatePlans();
      alert(
        `✅ Plan purchased successfully! Wallet debited ₦${plan.customPrice}`
      );
    } catch (err) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      // ✅ FIX: Remove the 's' to match the tab name exactly
      case "Transaction History":
        return (
          <WalletDashboard
            walletBalance={walletBalance}
            transactions={transactions}
            onRefresh={mutateWallet}
            loading={!fetchedTxns && !transactions.length}
          />
        );
      case "Fund Wallet":
